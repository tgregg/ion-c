...
IonEventStream::IonEventStream() {
    event_stream = new std::vector<IonEventBase *>();
}

IonEventStream::~IonEventStream() {
    for (int i = 0; i < event_stream->size(); i++) {
        IonEventBase *event = event_stream->at(i);
        if (event) {
...
IonEventBase * IonEventStream::append_new(ION_EVENT_TYPE event_type, ION_TYPE ion_type, ION_STRING *field_name,
                                          ION_STRING **annotations, SIZE num_annotations, int depth) {
    IonEventBase *event = new IonEventBase(event_type, ion_type, field_name, annotations, num_annotations, depth);
...
/* NOTE: it is the caller's responsibility to free the returned ION_STRING*. */
...
iERR read_value_stream(IonEventStream *stream, READER_INPUT_TYPE input_type, std::string pathname, LOOP_FN fn, std::string fn_name, ION_CATALOG *catalog)
{
    ...
    const char *fn_name_c_str = fn_name.c_str();
...
iERR write_scalar(hWRITER writer, IonEventBase *event) {
    iENTER;
    if (!event->value) {
...
iERR write_event(hWRITER writer, IonEventBase *event) {
    iENTER;
    if (event->field_name) {
