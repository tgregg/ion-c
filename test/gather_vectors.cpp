...
static std::string *test_concat_filename(std::string *dst, std::string path, std::string name)
{
    ...
    path_needs_separator = (pathlen > 0) ? (path[pathlen - 1] != ION_TEST_PATH_SEPARATOR_CHAR) : FALSE; // it there's no path we don't a separator
    file_needs_separator = (namelen > 0) ? (name[0] != ION_TEST_PATH_SEPARATOR_CHAR) : FALSE; // no name? no need to separate
...
static const std::string iontests_path = join_path("..", "ion-tests");
...
std::vector<std::string> _whitelist; // For debugging. Populate this with filenames to excercise only those.
...
iERR gather_files(TEST_FILE_TYPE filetype, TEST_FILE_CLASSIFICATION classification, std::vector<std::string> *files_out)
{
    ...
    IONCHECK(visit_files(iontests_path, classification_root_path, type_predicate, classification_predicate, title, files_out));
    ...
    if (dir) {
        ...
        while ((d = DIR_NEXT(dir)) != NULL)
        {
            ...
            if (   localfile.compare(".") != 0
                   && localfile.compare("..") != 0)
            {
                IONCHECK(visit_files(fullfilepath, localfile, type_predicate, classification_predicate, fn_name, files_out));
            }
        }
        DIR_CLOSE(dir);
    }
